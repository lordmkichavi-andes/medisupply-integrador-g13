...
image = f"{account}.dkr.ecr.{region}.amazonaws.com/routes:latest"
...
class RoutesService(Construct):
    ...
    def _create_task_definition(self):
        """Crear definición de tarea para Routes Service"""
        task_role = iam.Role(
            self, "RoutesServiceTaskRole",
        ...
        log_group = logs.LogGroup(
            self, "RoutesServiceLogGroup",
            log_group_name="/ecs/routes-service",
            retention=logs.RetentionDays.ONE_WEEK
        )
        task_definition = ecs.FargateTaskDefinition(
            self, "RoutesServiceTaskDefinition",
            cpu=256,
    ...
    def _create_service(self):
        """Crear servicio ECS para Routes Service"""
    ...
    def _configure_service_connections(self):
        ...
        self.database.connections.allow_default_port_from(
            ...
            description="Allow RoutesService Fargate to connect to PostgreSQL RDS"
        )

    def _configure_alb(self):
        """Configurar ALB para el servicio"""
        self.alb_listener.add_target_groups(
            "RoutesServiceTargetGroup",
            target_groups=[self.target_group],
            conditions=[elbv2.ListenerCondition.path_patterns(["/routes*"])],
            priority=150
        )
