...
import urllib.parse
# Removidas dependencias problemáticas: jwt, requests, pytz, ipaddress
...
def validate_cognito_jwt_real(token):
    """Validar JWT token con CACHE (sin dependencias problemáticas)"""
    try:
        ...
        try:
            ...
            claims = {
                ...
                'token_type': 'cognito_jwt_simplified'
            }
            ...
            logger.error(f"❌ Error decodificando JWT manualmente: {str(e)}")
            return None
...
def evaluate_security_with_cognito_data(user_profile, source_ip):
    ...
    try:
        ...
        if not business_check['valid']:
            risk_score += 0.7  # Incrementado de 0.3 a 0.7 para asegurar denegación en fin de semana
        ...
        if tolerance == 'low':
            risk_score *= 1.2  # Reducido de 1.3 a 1.2
        elif tolerance == 'high':
            risk_score *= 0.7  # Reducido de 0.8 a 0.7
        ...
        if role == 'admin':
            allow_threshold = 0.5  # Aumentado de 0.4 a 0.5 (más permisivo para admins)
            mfa_threshold = 0.8    # Aumentado de 0.7 a 0.8
        else:
            allow_threshold = 0.6  # Mantenido
            mfa_threshold = 0.9    # Aumentado de 0.8 a 0.9 (más permisivo)
