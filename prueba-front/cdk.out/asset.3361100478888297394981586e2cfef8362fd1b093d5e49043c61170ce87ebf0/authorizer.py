...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not token:
            token = headers_raw.get('Authorization') or headers_raw.get('authorization') or ''
...
def perform_security_validations(payload, headers):
    ...
    # 4. Validar acceso por grupos + país/hora por grupo
    return validate_cognito_groups_access(cognito_groups, headers, current_hour)
...
def validate_cognito_groups_access(cognito_groups, headers, current_hour):
    ...
    """
    # Reglas por grupo (países permitidos)
    country_raw = headers.get('x-country') or ''
    country = country_raw.strip().upper()
    allowed_countries_by_group = {
        'clientes': {'CO', 'PE', 'EC', 'MX'},
        'compras': {'CO', 'PE', 'EC', 'MX'},
        'logistica': {'CO', 'PE', 'EC', 'MX'},
        'ventas': {'CO', 'PE', 'EC', 'MX'},
    }

    # Si el usuario pertenece a algún grupo permitido, Allow (con reglas cuando aplique)
    mediSupply_groups = {
        'admin': {
 ...
 for group in cognito_groups:
        if group in mediSupply_groups:
            if group == 'admin':
                return {'allowed': True, 'reason': 'Access granted by admin group'}
            # País por grupo (compras/logistica/ventas/clientes)
            if group in allowed_countries_by_group:
                if not country:
                    return {'allowed': False, 'reason': 'Missing x-country header'}
                # Validación ISO-3166 alfa-2 simple (2 letras A-Z)
                if len(country) != 2 or not country.isalpha():
                    return {'allowed': False, 'reason': f'Invalid country code format: {country_raw}'}
                if country not in allowed_countries_by_group[group]:
                    return {'allowed': False, 'reason': f'Country {country} not allowed for {group}'}
                # Horario solo para clientes (demo)
                if group == 'clientes':
                    if current_hour < 6 or current_hour > 22:
                        return {'allowed': False, 'reason': f'Hour {current_hour} not allowed for clientes'}
                return {'allowed': True, 'reason': f'Access granted by {group} group (country ok)'}
            # Otros grupos sin reglas de país: permitir
...
def generate_policy(principal_id, effect, event):
    """Genera la política con ARN comodín para REQUEST authorizer"""
    method_arn = (event or {}).get('methodArn') or ''
    auth_type = (event or {}).get('type') or ''  # TOKEN o REQUEST
    # Para TOKEN authorizer usar el methodArn exacto; para REQUEST usar comodín
    if auth_type.upper() == 'TOKEN' and method_arn:
        resource_arn = method_arn
    else:
        # Formato: arn:aws:execute-api:region:account:apiId/stage/verb/resource
        try:
            parts = method_arn.split(':', 5)
            region = parts[3]
            account = parts[4]
            api_part = parts[5]  # apiId/stage/verb/resource
            api_id = api_part.split('/')[0]
            resource_arn = f"arn:aws:execute-api:{region}:{account}:{api_id}/*/*/*"
        except Exception:
