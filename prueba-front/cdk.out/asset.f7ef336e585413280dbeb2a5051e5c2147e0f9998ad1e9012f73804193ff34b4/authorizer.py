...
def perform_security_validations(payload, headers):
    ...
    if not cognito_groups:
        ...
        # Usar el username de Cognito (claim 'cognito:username' o 'username'), no el 'sub'
        user_name = payload.get('cognito:username') or payload.get('username')
        user_pool_id = os.environ.get('USER_POOL_ID')
        region = os.environ.get('AWS_REGION') or os.environ.get('AWS_DEFAULT_REGION')
        try:
            if user_name and user_pool_id and region:
                cognito = boto3.client('cognito-idp', region_name=region)
                resp = cognito.admin_list_groups_for_user(UserPoolId=user_pool_id, Username=user_name)
                cognito_groups = [g['GroupName'] for g in resp.get('Groups', [])]
                logger.info(f"Fetched groups from Cognito for username={user_name}: {cognito_groups}")
            else:
                logger.warning("Missing USER_POOL_ID/AWS_REGION/username to fetch groups from Cognito")
        except Exception as e:
