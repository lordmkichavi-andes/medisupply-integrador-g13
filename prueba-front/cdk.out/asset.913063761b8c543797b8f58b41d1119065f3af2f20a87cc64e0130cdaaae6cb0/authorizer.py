...
def validate_cognito_groups_access(cognito_groups, headers, current_hour):
    ...
    """
    # Reglas mínimas por grupo
    country = (headers.get('x-country') or '').upper().strip()
    allowed_countries_clientes = {'CO', 'PE', 'EC', 'MX'}
    ...
    for group in cognito_groups:
        if group in mediSupply_groups:
            ...
            if group == 'clientes':
                ...
                if country not in allowed_countries_clientes:
                    return {'allowed': False, 'reason': f'Country {country} not allowed for clientes'}
                if current_hour < 6 or current_hour > 22:
                    return {'allowed': False, 'reason': f'Hour {current_hour} not allowed for clientes'}
                return {'allowed': True, 'reason': 'Access granted by clientes group (country/hour ok)'}
            # Otros grupos: permitir sin reglas adicionales por ahora
